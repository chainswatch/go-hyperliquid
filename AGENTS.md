...
| **`Client`**                                                                             | `baseUrl`, `privateKey`, `defaultAddress`, `isMainnet`, `Debug`, `httpClient`, `keyManager`, `Logger` + methods `Request`, `SetPrivateKey`, `SetAccountAddress`, `IsMainnet`, `SetDebugActive`, `Endpoint` (via interfaces) | Shared HTTP layer (handles POST, headers, logging)      |
